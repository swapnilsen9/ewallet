  ...
  UserWallet.findOne({ userId: req.params.userId }, null, {
    ...
    .then((result2) => {
      if (result2.lastTxnId != req.body.lastTxnId) {
        var amt = parseFloat(req.body.amount) + parseFloat(result2.Amount);
        var body = {
  ...
  UserCredentials.findOne(
    ...
    .then((userCredentialresult) => {
      if (userCredentialresult === null) {
        ...
        console.log('userCredentialresult._id:: '+userCredentialresult._id);
        if (req.body.senderId != userCredentialresult._id) {
          UserWallet.findOne({ userId: userCredentialresult._id }, null, {
            limit: 1,
          })
            .exec()
            .then((userWalletRecieverResult) => {
              if (userWalletRecieverResult === null) {
                ...
                var amt =
                  ...
                  parseFloat(userWalletRecieverResult.Amount);
                var body = {
                  Amount: amt.toFixed(2),
                };
                UserWallet.findOneAndUpdate(
                  ...
                  .then((userWalletRecieverUpdateResult) => {
                    if (userWalletRecieverUpdateResult === null) {
                      res
                        .status(400)
                        .json({ error: "Money Could Not be Sent" });
                    } else {
                      UserWallet.findOne({ userId: req.body.senderId }, null, {
                        limit: 1,
                      })
                        .exec()
                        .then((userWalletSenderResult) => {
                          amt =
                            parseFloat(userWalletSenderResult.Amount) -
                            parseFloat(req.body.amount);
                          body = {
                            Amount: amt.toFixed(2),
                          };
                          UserWallet.findOneAndUpdate(
                            ...
                            .then((userWalletSenderUpdateResult) => {
                              if (
                                userWalletSenderUpdateResult != null &&
                                userWalletSenderUpdateResult != undefined &&
                                userWalletSenderUpdateResult._id != null &&
                                userWalletSenderUpdateResult._id != undefined
                              ) {
                                res
                                  .status(200)
                                  .json({ success: true, msg: "Amount Sent" });
                              } else {
                                res
                                  .status(400)
                                  .json({ error: "Money Could Not be Sent" });
          ...
          res
            .status(400)
            .json({ success: false, msg: "Can't Send Money to Yourself" });
        }
      }
    })
    .catch((err) => {
